...
interface Props {
  searchParams?: { genre?: string; region?: string };
}

export default function BrowsePage({ searchParams }: Props) {
  const [genre, setGenre] = useState(searchParams?.genre || 'All');
  const [region, setRegion] = useState(searchParams?.region || 'All');
  const [type, setType] = useState('All');

  let filtered = MOCK_VIDEOS;
  if (genre !== 'All') filtered = filtered.filter(v => v.genre === genre);
  if (region !== 'All') filtered = filtered.filter(v => v.region === region);
  if (type === 'Movies') filtered = filtered.filter(v => v.type === 'movie');
  if (type === 'Series') filtered = filtered.filter(v => v.type === 'series');
  ...
  return (
    <div className="min-h-screen bg-black text-white p-8">
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        <span className="text-gray-400 text-sm self-center">Found {filtered.length} titles</span>
      </div>

      {/* Grid */}
      {filtered.length === 0 ? (
        <div className="text-center text-gray-500 py-20">No videos match your filters.</div>
      ) : (
